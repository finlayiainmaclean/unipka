...
def process_dataset():
    ...
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MobleyLab/FreeSolv/refs/heads/master/database.txt",
        sep=";",
        skiprows=2,
    )
    ...
    if df.empty:
        print(
            "No data to evaluate for FreeSolv after dropping rows with missing values."
        )
        return None, {}
