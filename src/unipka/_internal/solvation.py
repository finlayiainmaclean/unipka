...
def get_boltzmann_weighted_average(
    *, properties: np.ndarray, energies: np.ndarray, T: float = 298
...
def embed(
    mol: Chem.Mol, /, *, num_confs: int = 100, rmsd_threshold: float | None = 0.25
) -> Chem.Mol:
...
def get_xtb_solvation(
    mol: Chem.Mol, charge: int = 0, num_cores: int = 1
) -> tuple[float, float]:
    """
    ...
    if shutil.which("xtb") is None:
        raise FileNotFoundError(
            "xtb is either not installed or not found in PATH. Please install xtb first. We recommend using `conda install conda-forge::xtb`."
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        proc = subprocess.run(
            cmd, cwd=tmpdir, capture_output=True, text=True, env=env, shell=True
        )
        ...
        if total_energy_match:
            ...
            raise ValueError(
                f"Could not find Gsolv in xTB output:\n{proc.returncode}\n{proc.stdout}\n{proc.stderr}"
            )
...
def get_solvation_energy(
    mol: Chem.Mol, /, *, num_rdkit_confs: int = 50, rmsd_threshold: float | None = 0.25
) -> float:
    mol = embed(
        Chem.AddHs(mol), num_confs=num_rdkit_confs, rmsd_threshold=rmsd_threshold
    )
    ...
    solvation_energy, _ = get_boltzmann_weighted_average(
        properties=solvation_energies, energies=total_energies
    )
