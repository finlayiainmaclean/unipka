...
def get_pattern_path(use_simple_smarts: bool = True) -> str:
    """Return the packaged SMARTS pattern file used for microstate enumeration."""
    return str(
        res.files("unipka.data").joinpath(
            "simple_smarts_pattern.tsv" if use_simple_smarts else "smarts_pattern.tsv"
        )
    )


def load_kpuu_model() -> LogisticRegression:
    """Load the pretrained Kp,uu logistic regression classifier."""
    weights_file = res.files("unipka.data").joinpath("weights.kpuu.npz")
    params = np.load(str(weights_file))
